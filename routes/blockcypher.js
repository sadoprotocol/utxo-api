...
router.all('/transaction', function(req, res, next) {
  blockcypher.transaction(req.body.txid).then(transaction => {
    res.json({
      success: true,
...
router.all('/balance', function(req, res, next) {
  blockcypher.balance(req.body.address).then(balance => {
    res.json({
      success: true,
...
router.all('/transactions', function(req, res, next) {
  blockcypher.transactions(req.body.address).then(transactions => {
    res.json({
      success: true,
...
router.all('/unspents', function(req, res, next) {
  blockcypher.unspents(req.body.address).then(unspents => {
    res.json({
      success: true,
